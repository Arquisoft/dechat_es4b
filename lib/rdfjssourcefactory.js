      ...
      if (res.status === 404) {
        ...
        parser.parse(body, (err, quad, prefixes) => {
          if (err) {
            ...
            const source = { match: function (s, p, o, g) { return require('streamify-array')(store.getQuads(s, p, o, g));}
            };
        deferred.resolve(source);}
        });
      }
    }).catch(reason => {console.warn(`No RDFJSSource was created for ${url}. File already deleted?`);
