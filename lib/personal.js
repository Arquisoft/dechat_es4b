...
class Personal {
  ...
  async loadFriendList(){
    if(this.friendList.length === 0){
      await fc.fetchAndParse(this.userWebId ).then((store) => { 
        this.searchFriendsOnList(store.statements);
        this.running = true;
      }, (err) => {this.running = false; console.log("could not fetch : "+err);});
    } 
    //console.log("CARGADOS");
    return "";
  }

  async loadInbox(openChat){
    this.myInbox = new Array();
    fc.readFolder(this.inbox).then((folder) => {
      for(let file of folder.files)
        this.myInbox.push(file);
      this.loadGroupNames(openChat);
    }, (err) => {console.log(err);});
  }

  async loadGroupNames(openChat){
    this.groupsLoaded = new Array();
    ...
    if(openChat){
      setTimeout(function(){$("#new-btn").click();}, 1000);
    }
  }
  ...
  */
  async searchFriendsOnList(possibleList) {
    ...
    for await(var friend of this.friendList){
      fc.readFile(this.inbox + friend.username + ".ttl").then((body) => {
        friend.body = body;
      }, () => {friend.body = "";});
    }
  }
