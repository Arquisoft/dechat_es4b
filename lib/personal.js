...
class Personal {
  ...
  async loadFriendBodies(){
    /*
    this.fileBodies = new Array();
    var friend = null
    for(friend of this.friendList){
      fc.readFile(this.inbox + friend.username + ".ttl").then(body => {
        this.fileBodies.push({"username": friend.username , "body": body});
        console.log(this.fileBodies);
      }, err => {});
    }
    */
  }
