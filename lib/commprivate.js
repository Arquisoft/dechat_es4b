...
class PrivateCommunication {
  ...
  */
  async loadMessages(personal, other, testing){
    ...
    fc.readFile(fileWithMessagesSentByMe).then(  body => {
     ...
     fc.updateFile(fileWithMessagesSentByMe, newBody).then(() => {}, err => {console.log(err)});
    }, err => {if(!testing){$("#addMessages").empty();}});
  ...
  */
  sendMessageToPod(personal, receiver, message){
    var body = "";
    ...
    fc.readFile(personal.inbox + receiver.username + ".ttl").then( success => {
      if(success.split("\n").length < 14){
        this.givePermission(personal.inbox, receiver);
      }
      ...
      fc.updateFile(personal.inbox + receiver.username + ".ttl", body).then( success => {
  ...
  async checkForNotifications(personal, other, nm){	  
    ...
    fc.readFile(fileWithMessagesSentByMe).then( body => {
      ...
      this.checkHisLastMessage(fileWithMessagesSentByTheOtherUser, other, dateRead, nm);
    }, err => {this.checkHisLastMessage(fileWithMessagesSentByTheOtherUser, other, "NEVER", nm);});

    
  }

  checkHisLastMessage(fileWithMessagesSentByTheOtherUser, other, dateRead, nm){
    fc.readFile(fileWithMessagesSentByTheOtherUser).then( body => {
      var res = body.split("\n\n\n");
      //res[>=2] => MENSAJES (MXXXX)
      for(var i=2; i<res.length; i++){
        ...
        //Get Message
        var aux = lines[3].split('\t"')[1];
        var lastMessage = aux.replace('"\t.', "");
      }
      console.log(other.username, dateRead);
      if(dateRead !== ""){
        if(dateRead === "NEVER" || dateLastMsg > dateRead){
  ...
  */
  async communicationEstablished(personal, receiver){
    var exists = true;
    await fc.readFile(personal.inbox+receiver.username+".ttl").then( () => {}, err => exists = false );
    await fc.readFile(receiver.inbox+personal.username+".ttl").then( ()=> {}, err => exists = false );
    return exists;
  }
