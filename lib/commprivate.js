...
class PrivateCommunication extends BaseCommunication {
  constructor(fetch) {
    super();
  ...
  */
  async loadMessages(personal, other, testing){
    ...
    if(testing){
        fileWithMessagesSentByMe = "https://enriquead.solid.community/public/mariodiaz98.ttl";
        fileWithMessagesSentByTheOtherUSer = "https://enriquead.solid.community/public/mariodiaz98.ttl";   
    }
    ...
    await  fc.readFile(fileWithMessagesSentByMe).then((body) => {
      var res = body.split("\n\n\n");
      //res[1] => CHAT INFO
      var chat = res[1];
    ...
    await fc.readFile(fileWithMessagesSentByTheOtherUSer).then((body) => {
     var res = body.split("\n\n\n");
     //res[>=2] => MENSAJES (MXXXX)
     for(var i=2; i<res.length; i++){
      var lines = res[i].split("\n");
  ...
  showMessages(messages, other, me, testing){
    ...
    for(var i=messages.length-1; i>=0; i--){
      ...
      if (format[format.length-1] === "jpg" || format[format.length-1] === "gif" || format[format.length-1] === "png" || mesg.startsWith("data:image")){
		...
		if ( mesg.startsWith("https://") || mesg.startsWith("http://"))
			mesgToAdd = "<p><a href="+mesg+" target='_blank'>"+mesg+"</p>";
  ...
  */
  ...
  async checkHisLastMessage(fileWithMessagesSentByTheOtherUser, other, dateRead, nm, personal){
    var lastMessage = "";
  ...
  */
  ...
  getMessageToAdd(){
      return this.messageToAdd;
  }
  getDatesRead(){
      return this.datesRead;
  }
