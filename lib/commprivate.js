  ...
  */
  async loadMessages(personal, other, testing){
    ...
    fc.readFile(fileWithMessagesSentByMe).then(  body => {
     ...
     fc.updateFile(fileWithMessagesSentByMe, newBody + this.messageToAdd).then(() => {this.messageToAdd = ""}, err => {console.log(err)});
    }, err => {noFileForMe = true;});
  ...
  */
  sendMessageToPod(personal, receiver, message){
    ...
    fc.readFile(personal.inbox + receiver.username + ".ttl").then( success => {
      ...
      fc.deleteFile(personal.inbox + receiver.username + ".ttl").then(success => {
        console.log(success);
  ...
  async checkForNotifications(personal, other, nm){	  
    ...
    }, err => {this.checkHisLastMessage(fileWithMessagesSentByTheOtherUser, other, "NEVER", nm, personal);});   
  }
