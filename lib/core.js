...
class ChatCore {
  ...
  async createGroup(personal, friendsGroup) {
    ...
    for(friend in friendsGroup){
      this.commGroup.sendFirstMessage(personal, friend, rand);
    }
    this.commGroup.sendFirstMessage(personal, personal, rand);
  ...
  async loadMessages(personal){
    ...
    // Group chat
    if(receiver == null){
      this.commGroup.loadMessages(personal.username);
    } else{ // Private chat
  ...
  async clearInbox(personal){
    //  

    /*
    const resources = await this.getAllResourcesInInbox(await this.getInboxUrl(personal.userWebId));
