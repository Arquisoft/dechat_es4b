...
class GroupCommunication {
  ...
  */
  async loadMessages(personal, myUrlFile, testing){
    ...
    var group;
    if(testing){
      group = {"file": "chat", "name": "testChat", "type": "group", "urlCore": "urlRemote"}
    ...
    }, err => {$("#addMessages").empty(); if(err.includes("403")){ console.log("You have been removed from this group")}else if(err.includes("404")){ console.log("This group has been deleted")} else{console.log(err);}});
  }
  ...
  */
  ...
  createCoreFile(personal, rand, groupName, friendsGroup){
    ...
    fc.updateFile(urlCore + ".acl", this.templatePermission(friendsGroup, "groupcore_" + rand+ ".ttl")).then( () => {this.alerts.alertGroupCreated(groupName); personal.loadInbox();}, err => console.log(err));
  }
