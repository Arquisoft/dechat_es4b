    ...
    fc.readFile(group.urlCore).then(  body => {
	  ...
	  for ( var i=0; i<messages.length; i++){
        ...
        while (copyMesg != mesgToAdd){
          copyMesg = mesgToAdd;
          mesgToAdd = mesgToAdd.replace(re, '<img src="/src/img/emotes/$1.jpg" width="30" heigth="30">');
        } 
			}
			if ( messages[i].sender != personal.username ){			
				...
				var toAppend = '<div class="outgoing_msg" id="outgoing_msg">'+								
									...
									'<p>'+mesgToAdd+'</p>' + '<span id="userName" class="badge badge-secondary">'+messages[i].sender+'</span>'+
									'</div>'+
								'</div>';
				$("#addMessages").append(toAppend);
			}
